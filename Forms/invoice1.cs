...
namespace SaleManage
{
    public partial class invoice1 : Form
    {
        ...
        private void invoice1_Load(object sender, EventArgs e)
        {
            ...
            string companyInfo = sysRepo.GetSetting("company_info") ?? "";
            string bankInfo = sysRepo.GetSetting("bank_info") ?? "";
            string taxRate = sysRepo.GetSetting("tax_rate") ?? "0";

            int subTotal = CalculateTotal(salesDt);
            int tax = (int)(subTotal * int.Parse(taxRate) / 100.0);
            ...
            reportViewer1.LocalReport.SetParameters(new ReportParameter[]
            {
                ...
                new ReportParameter("IssueDate",       _issueDate.ToShortDateString()),
                new ReportParameter("CompanyInfo",     companyInfo),
                new ReportParameter("TotalAmount",     totalAmount.ToString()),
                new ReportParameter("PaymentDeadline", _paymentDeadline.ToShortDateString()),
                new ReportParameter("BankInfo",        bankInfo),
                new ReportParameter("TaxRate",         taxRate),
            });
